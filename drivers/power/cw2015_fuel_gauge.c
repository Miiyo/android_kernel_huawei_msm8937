...
#define VOLTAGE_FOR_LOW_TEMP_CLOSE 3500
#define VOLTAGE_FORCE_CLOSE        3050
#define VOLTAGE_FORCE_CAP_2        3400
...
#define REG_IC_TEMP_H 0x0d
#define REG_IC_TEMP_L 0x0c
#define CW_IC_TEMP_VBASE 3010
#define CW_IC_TEMP_RATIO 256
#define CW_IC_TEMP_CAL_DIVID 8
#define CW_IC_TEMP_BASE 25
#define TEMP_THRESHOLD 150
#define TEMP_MULTIPLY 100
#define TEMP_DIVISION 10
#define LOW_TEMP_THRESHOLD 50
#define TEMP_STEP 2
int g_cw_ic_temp = 0;
int cw_get_ic_temp(struct cw_battery *cw_bms)
{
	int ret = 0;
	unsigned char vtemph = 0;
	unsigned char vtempl = 0;
	...
	pr_info("\n");
	ret = cw_read(cw_bms->client, REG_IC_TEMP_H, &vtemph);
	if(ret < 0)
		return 0;

	pr_info("\n");
	ret = cw_read(cw_bms->client, REG_IC_TEMP_L, &vtempl);
	if(ret < 0)
		return 0;

	g_cw_ic_temp = (CW_IC_TEMP_BASE*TEMP_MULTIPLY - ((vtemph*CW_IC_TEMP_RATIO + vtempl) - CW_IC_TEMP_VBASE)*TEMP_MULTIPLY / CW_IC_TEMP_CAL_DIVID) / TEMP_DIVISION;
	pr_info("cw_get_ic_temp temp = %d\n", g_cw_ic_temp);
	return g_cw_ic_temp;
}
...
static int cw_capacity_second_change(int cap, struct cw_battery *cw_bms)
{
	...
	static int high_voltage_cap_mark = 0;
	static int voltage_cap_2_force = 0;
	int cap_second = 0;
	int ret = 0;
	...
	int now_vol;
	int last_vol;
	int shutdown_vol = VOLTAGE_FORCE_CAP_2;
	int now_temp;
	last_vol = cw_bms->voltage;
	now_vol = cw_get_voltage(cw_bms);
	now_temp = get_batt_temp(cw_bms);
	pr_info("cw_low_voltage last_vol = %d now_vol = %d\n", last_vol, now_vol);

	if(cw_bms->charger_mode > 0 && voltage_cap_2_force == 1){
		voltage_cap_2_force = 0;
	}
	if(voltage_cap_2_force == 1){
	...
	if(cap <= CAP_2 && cw_bms->charger_mode == 0 && now_temp <= LOW_TEMP_THRESHOLD && cw_bms->capacity >= CAP_3 &&
		(last_vol >= VOLTAGE_FOR_LOW_TEMP_CLOSE || now_vol >= VOLTAGE_FOR_LOW_TEMP_CLOSE)){
		cap_second = CAP_3;
		high_voltage_cap_mark=1 ;
		pr_err("join discharge lock 3\n");
		return cap_second;
	}

	if( cap <= CAP_2 && cw_bms->charger_mode > 0 && high_voltage_cap_mark == 1 ){
		cap_second = CAP_3;
		pr_err("join charging lock 3\n");
		return cap_second;
	}

	if( cap <= CAP_2 && cw_bms->charger_mode == 0 && high_voltage_cap_mark == 1 
		&& last_vol < VOLTAGE_FOR_LOW_TEMP_CLOSE && now_vol < VOLTAGE_FOR_LOW_TEMP_CLOSE 
		&& last_vol > VOLTAGE_FORCE_CLOSE && now_vol > VOLTAGE_FORCE_CLOSE){
		cap_second = CAP_2;
		return cap_second;
	}

	high_voltage_cap_mark = 0;

	if(last_vol <= VOLTAGE_FORCE_CLOSE && now_vol <= VOLTAGE_FORCE_CLOSE){
		cap_second = 0;
		return cap_second;
	}


	shutdown_vol = (now_temp - TEMP_THRESHOLD) * TEMP_STEP + VOLTAGE_FORCE_CAP_2;
	if(shutdown_vol > VOLTAGE_FORCE_CAP_2){
		shutdown_vol = VOLTAGE_FORCE_CAP_2;
	}
	pr_info("cw_low_voltage now_temp = %d  shutdown_vol = %d\n", now_temp, shutdown_vol);
	if((cap >= CAP_3 && cap <= CAP_6)  && cw_bms->charger_mode == 0 
		&& (last_vol <= shutdown_vol && now_vol <= shutdown_vol)){
		cap_second = cw_bms->capacity - 1;
		voltage_cap_2_force = 1;
		return cap_second;
	}
